 ...
 *
 * Disassembly of DSDT.aml, Mon Sep 17 17:08:02 2018
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x00021B10 (138000)
 *     Revision         0x02
 *     Checksum         0x53
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "SKL     "
...
DefinitionBlock ("", "DSDT", 2, "LENOVO", "SKL     ", 0x00000000)
{
    ...
    External (_SB_.IETM.DPTE, UnknownObj)    // (from opcode)
    ...
    External (_SB_.PCI0.LPCB.EC__.HKEY.TSSS, MethodObj)    // 0 Arguments (from opcode)
    ...
    External (_SB_.PCI0.RP01.PEGP.NVST, MethodObj)    // 0 Arguments (from opcode)
    ...
    External (DX2H, MethodObj)    // 2 Arguments (from opcode)
    ...
    Scope (\_SB.PCI0.I2C0)
    {
        Device (TPD0)
        {
            ...
            Method (_STA, 0, NotSerialized)  // _STA: Status
            {
                If (LOr (LEqual (SDS0, 0x05), LOr (LEqual (SDS0, 0x01), LOr (LEqual (SDS0, 0x02), LEqual (SDS0, 0x06)))))
                {
                    Return (0x0F)
                }
            ...
            Name (BUF0, ResourceTemplate()
{
    IRQNoFlags() { 0, 8, 11, 15 }
        ...
        Device (MOU)
        {
            ...
            Method (MHID, 0, NotSerialized)
            {
                ...
                {
                    Store (0x7B00AE30, _HID)
                }
                ElseIf (\_SB.PCI0.LPCB.NFCD)
                {
                    Store (0x8F00AE30, _HID)
                }
                Else
                {
                    Store (0x9000AE30, _HID)
                }
            }
        }
    ...
    Scope (\_SB.PCI0.LPCB.EC)
    {
        ...
        {
            If (\_SB.PCI0.LPCB.EC.HKEY.MHKK (0x03, 0x4000))
            {
                \_SB.PCI0.LPCB.EC.HKEY.TSSS ()
            }
        }
        ...
        {
            Fatal (0x01, 0x80010000, 0x00011DE5)
        }
            ...
            Method (SSSS, 1, NotSerialized)
            {
                If (LEqual (And (\SHDW, 0x80), 0x00))
                {
                    If (LEqual (Arg0, 0x00))
                    {
                        ...
                        {
                            \UCMS (0x1E)
                            Store (And (\SHDW, Not (0x01)), \SHDW)
                            \_SB.PCI0.LPCB.EC.HKEY.MHKQ (0x130F)
                            Return (0x00)
                        }
                    }
            ...
            If (LAnd (\WNTF, \TATC))
            {
                Switch (ToInteger (And (Local0, 0x01FF)))
                {
                    ...
                    Case (0x01)
                    {
                        ...
                        Switch (Local2)
                        {
                            ...
                            Case (0x0D)
                            {
                                If (LAnd (LLessEqual (Local3, 0x08), LGreaterEqual (Local3, 0x01)))
                                {
                                    ...
                                    If (LAnd (LNotEqual (Local3, 0x02), LAnd (LNotEqual (Local3, 0x07), LNotEqual (Local3, 0x08))))
                                    {
                        ...
                        Or (ShiftLeft (\VTIO, 0x02), Local5, Local5)
                        Or (ShiftLeft (\VMYH, 0x03), Local5, Local5)
                        Or (ShiftLeft (\VSTP, 0x04), Local5, Local5)
                        Or (ShiftLeft (\VCQH, 0x05), Local5, Local5)
                        Or (ShiftLeft (\VDCC, 0x06), Local5, Local5)
                    ...
                    Case (0x05)
                    {
                        If (LNotEqual (0x00, 0x01))
                        {
                            Store (ShiftLeft (0x05, 0x08), Local1)
                            Or (Local1, ShiftLeft (0x010E, 0x14), Local1)
                        }
                    ...
                    Case (0x0100)
                    {
                        Store (ShiftLeft (0x1001, 0x10), Local1)
                        Or (Local1, 0x01, Local1)
                    }
                    Case (0x01FF)
                    {
